...
public class MostrarMatacantfacció {
            ...
            try {
                if (!option.equals("100") && faccionesnumber.contains(Integer.parseInt(option))) {
                    ResultSet newrs = UtilsSQLite.querySelect(conn,
                            "SELECT * FROM Personatge WHERE idFaccio = " + option + " ORDER BY atac DESC LIMIT 1;");
    ...
    public void mostrarresultado(ResultSet rs, String nombref) throws SQLException {
        System.out.println("Personaje con más daño de la facción " + nombref + " és:" + rs.getString("nom")
                + " con un daño de: " + rs.getInt("atac"));
        System.out.println();
    }
